...
import {
  CAPAS, FUENTE_META, idsDeCapa, idsDeNodo,
  type Capa, type Nodo, type Check,
} from "./sirPlusCapas";
import {
...
function Punto({ estado, size = 10 }: { estado: EstadoKey; size?: number }) {
...
function BotonEstado({ estado, onClick, compacto }: { estado: EstadoKey; onClick: () => void; compacto?: boolean }) {
  const m = ESTADOS[estado];
  return (
    <button
      onClick={onClick}
      ...
      >
        ...
        <div style={{ padding: "16px 22px 40px" }}>
          ...
          {nodo.checks.map((c: Check) => {
            ...
            return (
              <div key={c.id} style={{
                background: T.card, border: `1px solid ${est === "sin_verificar" ? T.border : ESTADOS[est].color + "35"}`,
